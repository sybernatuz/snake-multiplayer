class Snake:
	...
	def display_snake_body(self, can):
		for i in range(len(self.snake)-1, 0, -1):
			...
			can.create_oval(self.snake[i][0], self.snake[i][1], self.snake[i][0]+10, self.snake[i][1]+10,outline='green', fill='black')
	...
	def check_collision(self, snake2, flag):
		for i in range(len(snake2)):
			check_x = self.snake[0][0] > snake2[i][0]-7 and self.snake[0][0] < snake2[i][0]+7
			check_y = self.snake[0][1] > snake2[i][1]-7 and self.snake[0][1] < snake2[i][1]+7
